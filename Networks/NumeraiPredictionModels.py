...
class ResidualBlock(nn.Module):
    def __init__(self, residual_dim=512, brodcast_dim=1048, dropout_prob=0.05, activation_fnc=nn.Tanh, device='cuda'):
...
class ResidualFeatureEncoder(nn.Module):
    def __init__(self, expert_num, num_residuals=5, input_dim=313, output_dim= 256, residual_dim=512, dataset_name="Dataset_568", device='cuda'):
...
class ExpertDecoder(nn.Module):
    def __init__(self, num_experts = 15, num_residuals=5, input_dim=310, output_dim=5, residual_input_dim=313, residual_output_dim= 256, residual_dim=512, dropout_probs=0.05, load_default=False, dataset_name="Dataset_563", device='cuda'):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Sigmoid(), # Not Cross-Entropy Loss
        )
        self._ds_name = dataset_name
    ...
    def predict(self, x):
        self.eval()
        output = self(x)
        cls_pred = th.argmax(output, dim=1)
        return cls_pred.data.cpu() / 4

    def load_checkpoint(self):
        if os.path.exists(os.path.join(self.save_path, self._file_name)):
            self.load_state_dict(th.load(os.path.join(self.save_path, self._file_name)))
    
    def save_checkpoint(self, file=None):
        print(f'[ExpertDecoder] Saving Checkpoint...')
        if file != None:
            th.save(self.state_dict(), file)
        else:
            th.save(self.state_dict(), self.save_path + "/" + self._file_name)
